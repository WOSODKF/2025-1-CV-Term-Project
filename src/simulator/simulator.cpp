...
void Simulator::callback(const mjModel *m, mjData *d)
{
    for (int id = 0; id < _agent_num; id++)
    {
        ...
        + if converged to grasp position: enable equality
        */
        for (int id = 0; id < _agent_num; id++)
        {
            set_control(d, _last_control[id], _mj_ID[id]._first_act_ID);
        }
    }
}
...
void Simulator::set_eq_data(mjModel *m, mjData *d)
{
    for (int id = 0; id < _agent_num; id++)
    {
        int eq_id = _mj_ID[id]._grasp_equality_ID;
        int B2_id = _mj_ID[id]._first_body_ID + 3;
        ...
        if (!d->eq_active[eq_id])
        {
            ...
            int B0_id = _mj_ID[id]._first_body_ID;
            int free_qpos_id = _mj_ID[id]._first_qpos_ID;
